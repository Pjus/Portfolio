...
def cafe():
    url = "https://cafe.naver.com/twcenter"  # input("접속하기 원하는 카페 url을 입력하세요:")
    # 본인 계정 비번 입력
    id = 'wnstjd117'
    pw = 'qkrEkdRh71672'
    ...
    """
    while (k <= int(page)):#main-area > div.prev-next > a.on#main-area > div.prev-next > a:nth-child(4)
        next_page = driver.find_elements_by_css_selector('#main-area > div.prev-next > a:nth-child(' + str(k) + ')')
        for nn in next_page:
            nn.click()
        k = k + 1
        print("page: {}".format(k))
        ...
        while (i <= 15):
            ...
            # main-area > div.article-board.result-board.m-tcol-c > table > tbody > tr:nth-child(1) > td.td_article > div.board-list > div > a.article
            url = driver.find_elements_by_css_selector('#main-area > div.article-board.result-board.m-tcol-c > table > '
                                                       'tbody > tr:nth-child(' + str(i) + ') > td.td_article > div.board-list > div > a.article')
            # print(url)
            for li in url:
                url_li = li.get_attribute('href')
